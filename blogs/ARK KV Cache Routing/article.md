...
![ARK Figure 1：蓝框为网络拓扑，橙框为路径预留控制器，绿框为控制平面与 KV 数据平面](./pic/ark-figure-1-annotated.png)

- **蓝色 (a)：网络拓扑。** GPU host 连接 leaf，再通过多条 spine 路径形成 ECMP 候选路径。
- **橙色 (b)：路径预留控制器。** 控制器查询 path table，选择并预留路径，检测并处理并发 reservation race，传输结束后释放路径。
- **绿色 (c)：控制平面与 KV 数据平面。** Host 之间广播 reservation，真正的 KV flow 则沿选定的 non-overlapped path 进入网络。
